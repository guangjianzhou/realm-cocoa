...
template<typename T>
static NSNumber *get_boxed(__unsafe_unretained RLMObjectBase *const obj, NSUInteger index) {
    RLMVerifyAttached(obj);
...
static inline void RLMSetValue(__unsafe_unretained RLMObjectBase *const obj, NSUInteger colIndex,
    ...
    if (!val) {
        ...
        NSString *objectClassName = obj->_info->propertyForTableColumn(colIndex).objectClassName;
        if (![objectClassName isEqualToString:valSchema.className]) {
            @throw RLMException(@"Can't set object of type '%@' to property of type '%@'",
                                valSchema.className, objectClassName);
        }
...
static IMP RLMAccessorGetter(RLMProperty *prop, RLMAccessorCode accessorCode) {
    ...
    switch (accessorCode) {
        ...
        case RLMAccessorCodeIntObject:
            return imp_implementationWithBlock(^(__unsafe_unretained RLMObjectBase *const obj) {
                return get_boxed<int64_t>(obj, index);
            });
        case RLMAccessorCodeFloatObject:
            return imp_implementationWithBlock(^(__unsafe_unretained RLMObjectBase *const obj) {
                return get_boxed<float>(obj, index);
            });
        case RLMAccessorCodeDoubleObject:
            return imp_implementationWithBlock(^(__unsafe_unretained RLMObjectBase *const obj) {
                return get_boxed<double>(obj, index);
            });
        case RLMAccessorCodeBoolObject:
            return imp_implementationWithBlock(^(__unsafe_unretained RLMObjectBase *const obj) {
                return get_boxed<bool>(obj, index);
            });
...
id RLMDynamicGet(__unsafe_unretained RLMObjectBase *const obj, __unsafe_unretained RLMProperty *const prop) {
    ...
    switch (accessorCodeForType(prop.objcType, prop.type)) {
        case RLMAccessorCodeByte:         return @((char)get<int64_t>(obj, index));
        case RLMAccessorCodeShort:        return @((short)get<int64_t>(obj, index));
        case RLMAccessorCodeInt:          return @((int)get<int64_t>(obj, index));
        case RLMAccessorCodeLong:         return @((long)get<int64_t>(obj, index));
        case RLMAccessorCodeLongLong:     return @(get<int64_t>(obj, index));
        case RLMAccessorCodeFloat:        return @(get<float>(obj, index));
        case RLMAccessorCodeDouble:       return @(get<double>(obj, index));
        case RLMAccessorCodeBool:         return @(get<bool>(obj, index));
        ...
        case RLMAccessorCodeIntObject:    return get_boxed<int64_t>(obj, index);
        case RLMAccessorCodeFloatObject:  return get_boxed<float>(obj, index);
        case RLMAccessorCodeDoubleObject: return get_boxed<double>(obj, index);
        case RLMAccessorCodeBoolObject:   return get_boxed<bool>(obj, index);
